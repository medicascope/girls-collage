      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {stats.map((stat, index) => (
            <div
              key={index}
              className={`bg-gradient-to-br ${stat.bgColor} p-8 rounded-2xl card-shadow group hover:scale-105 transition-transform duration-300`}
            >
              <div className="text-center">
                <div
                  className={`w-16 h-16 bg-gradient-to-r ${stat.color} rounded-full flex items-center justify-center mx-auto mb-4 text-white group-hover:scale-110 transition-transform duration-300`}
                >
                  {stat.icon}
                </div>
        ...
        <div className="bg-white p-8 rounded-2xl card-shadow">
          <h3 className="text-2xl font-bold text-gray-800 mb-8 text-center">
            توزيع أعضاء هيئة التدريس حسب الأقسام
          </h3>
          ...
          <div className="space-y-4">
            {departments.map((dept, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex flex-col items-center text-center mb-6 md:flex-row md:items-center md:text-right md:space-x-0 md:space-x-reverse md:space-x-4"
                >
                  <div className="w-40 text-sm font-medium text-gray-700 flex-shrink-0 mb-2 md:mb-0">
                    {dept.name}
                  </div>
                  <div className="flex flex-col md:flex-row flex-1 w-full items-center">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-white p-6 rounded-xl card-shadow text-center">
            ...
            <div className="text-sm text-gray-500 mt-1">
              {(((totalFaculty * 0.3) / totalFaculty) * 100).toFixed(0)}% من
          ...
          <div className="bg-white p-6 rounded-xl card-shadow text-center">
            ...
            <div className="text-sm text-gray-500 mt-1">
              {(((totalFaculty * 0.4) / totalFaculty) * 100).toFixed(0)}% من
              إجمالي أعضاء هيئة التدريس
            </div>
          </div>
