...
export default function ProgramContent({ program, activeTab }) {
  if (!program) return <div className="p-6 text-center text-gray-500">لا توجد بيانات</div>;

  // Safe data extraction with PortableText handling
  const safeOverview = extractText(program?.overview) || 'لا توجد معلومات متاحة حالياً';
  const safeFees = program?.fees || {};
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 leading-relaxed">{safeOverview}</p>
        </div>
        
        {program?.objectives && Array.isArray(program.objectives) && program.objectives.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {program.objectives.slice(0, 5).map((objective, index) => (
                <div key={`objective-${index}`} className="flex items-start space-x-3 space-x-reverse">
  ...
  if (activeTab === 'details') {
    return (
      <div className="space-y-6">
        <h4 className="text-xl font-bold mb-4 text-gray-800">تفاصيل البرنامج</h4>
          ...
          <div className="bg-blue-50 p-6 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{extractText(program?.level) || 'غير محدد'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">المدة:</span>
                <span className="font-medium">{extractText(program?.duration) || 'غير محدد'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">الدرجة:</span>
                <span className="font-medium">{extractText(program?.degree) || 'غير محدد'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-green-50 p-6 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{program?.capacity || 'غير محدد'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">الطلاب الحاليين:</span>
                <span className="font-medium">{program?.currentStudents || 'غير محدد'}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600 mb-2">
                {extractText(safeFees.tuition) || safeFees.tuition || 'غير محدد'}
              </div>
              <div className="text-gray-600 text-sm">الرسوم الدراسية</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600 mb-2">
                {extractText(safeFees.total) || safeFees.total || 'غير محدد'}
              </div>
              <div className="text-gray-600 text-sm">الإجمالي السنوي</div>
            </div>
          </div>
          
          <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-yellow-800 text-sm">
              <strong>ملاحظة:</strong> الرسوم قابلة للتغيير حسب اللوائح المعتمدة.
            </p>
          </div>
        </div>
      </div>
